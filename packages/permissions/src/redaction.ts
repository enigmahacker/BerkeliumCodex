...
export class SecretRedactor {
  private patterns: RedactionPattern[] = [
    ...
    // Password / Secret in assignment or env var
    {
      ...
      replace: (match) => {
        ...
        if (!separatorMatch || separatorMatch.index === undefined) return match;
  ...
  public containsSecret(input: string): boolean {
  ...
  public extractSecretTypes(input: string): string[] {
  ...
  public redactObject<T>(target: T): T {
    if (target === null || target === undefined) {
      return target;
    }

    if (typeof target === 'string') {
      return this.redact(target).redacted as unknown as T;
    }

    if (Array.isArray(target)) {
      return target.map((item) => this.redactObject(item)) as unknown as T;
    }
