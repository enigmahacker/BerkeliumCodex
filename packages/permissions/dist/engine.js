...
export class PermissionEngine {
    ...
    checkFilesystem(request) {
        ...
        if (act.includes('delete') || act.includes('unlink') || act.includes('remove')) {
            if (!isInside) {
                ...
                if (pol === 'deny')
                    return { allowed: false, requiresPrompt: false, risk: 'critical', reason: 'Deleting outside workspace is denied by policy' };
            ...
            if (pol === 'deny')
                return { allowed: false, requiresPrompt: false, risk: 'critical', reason: 'File deletion denied by policy' };
            return { allowed: false, requiresPrompt: true, risk: 'medium' };
        }
        return { allowed: false, requiresPrompt: true, risk: 'medium' };
    }
    checkShell(request) {
        ...
        if (isKnownSafe && !hasChainOperators) {
            ...
            if (pol === 'allow')
                return { allowed: true, requiresPrompt: false, risk: 'low' };
            if (pol === 'deny')
                return { allowed: false, requiresPrompt: false, risk: 'low', reason: 'Shell execution denied' };
            return { allowed: false, requiresPrompt: true, risk: 'low' };
        }
        ...
        if (pol === 'allow')
            return { allowed: true, requiresPrompt: false, risk: 'medium' };
        if (pol === 'deny')
            return { allowed: false, requiresPrompt: false, risk: 'high', reason: 'Shell command denied by policy' };
        return { allowed: false, requiresPrompt: true, risk: 'medium' };
    }
    checkGit(request) {
        const act = request.action.toLowerCase();
    ...
    checkNetwork(request) {
        ...
        if (pol === 'allow')
            return { allowed: true, requiresPrompt: false, risk: 'medium' };
        if (pol === 'deny')
            return { allowed: false, requiresPrompt: false, risk: 'medium', reason: 'Network access denied' };
        return { allowed: false, requiresPrompt: true, risk: 'medium' };
    }
    checkMcp(request) {
        return {
