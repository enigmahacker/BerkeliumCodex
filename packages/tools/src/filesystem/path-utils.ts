...
export function resolveSafeWorkspacePath(workspaceRoot: string, targetPath: string): string {
    ...
    try {
      ...
      if (!isInside) {
        throw new SecurityPathError(
          `Symlink escape blocked: "${targetPath}" links to "${canonicalTarget}" outside authorized workspace root "${canonicalRoot}"`,
          targetPath
        );
      }
  ...
  if (!isInside) {
    throw new SecurityPathError(
      `Path traversal blocked: "${targetPath}" resolves outside authorized workspace root "${canonicalRoot}"`,
      targetPath
    );
  }
