export class ResponseNormalizer {
    static createAccumulator(model, provider) {
        let fullText = '';
        let fullReasoning = '';
            ...
            processChunk(chunk) {
                if (chunk.type === 'token' && chunk.text) {
                    ...
                    if (chunk.toolCall.id)
                        curr.id = chunk.toolCall.id;
                    if (chunk.toolCall.name)
                        curr.name = chunk.toolCall.name;
                    if (chunk.toolCall.argumentsDelta)
                        curr.argsString += chunk.toolCall.argumentsDelta;
                }
    ...
    static normalizeOpenAIChunk(raw) {
        const chunks = [];
