...
import { ProviderRouter, Message, ResponseNormalizer } from '@berkelium/providers';
...
export class MissionRunner {
   ...
   */
  public decomposeGoal(goal: string): MissionTask[] {
  ...
  public async executeMission(
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      this.eventBus.emit({
        id: crypto.randomUUID(),
        type: 'mission_task_started' as any,
        sessionId: missionId,
        timestamp: Date.now(),
        taskId: task.id,
...
Complete this objective using appropriate tools. Once the objective is met, provide a concise result summary.`;
      ...
      this.eventBus.emit({
        id: crypto.randomUUID(),
        type: 'mission_task_completed' as any,
        sessionId: missionId,
        timestamp: Date.now(),
        taskId: task.id,
        status: task.status,
