...
export class HardwareDetector {
     ...
     */
    calculateBudget(currentlyLoadedBytes = 0) {
     ...
     */
    canLoadModel(model, currentlyLoadedBytes = 0) {
        ...
        if (estimatedModelMem > budget.remaining_bytes) {
            ...
            return (`Insufficient memory to load ${model.id}.\n` +
        ...
        if (model.hardware_requirements.compatible_runtimes.length > 0) {
            ...
            const compatible = model.hardware_requirements.compatible_runtimes.some(rt => {
                if (rt === 'mlx')
                    return hasMLX;
                if (rt === 'gguf')
                    return true; // GGUF runs on any platform
                if (rt === 'cpu')
                    return true;
                return false;
            });
    ...
    detectGPUCores() {
        try {
            const output = execSync('system_profiler SPDisplaysDataType 2>/dev/null | grep "Total Number of Cores"', { encoding: 'utf-8' }).trim();
    ...
    detectCoreTopology() {
        try {
            const perfCores = parseInt(execSync('sysctl -n hw.perflevel0.logicalcpu', { encoding: 'utf-8' }).trim(), 10);
            const effCores = parseInt(execSync('sysctl -n hw.perflevel1.logicalcpu', { encoding: 'utf-8' }).trim(), 10);
...
function suggestSmallerModel(parameters) {
    const sizeMap = {
