...
export class GitCommand {
    ...
    static isDestructive(cmd) {
        return GitCommand.DESTRUCTIVE_PATTERNS.some((pattern) => pattern.test(cmd));
    }
    static async run(themeManager, subaction, extraArgs = [], workspaceRoot = process.cwd()) {
        ...
        try {
            switch (action) {
                ...
                case 'log': {
                    ...
                    const logOutput = execSync(`git log -n ${isNaN(limit) ? 10 : limit} --format="%C(auto)%h %ad %s" --date=short`, {
                        cwd: workspaceRoot,
                        encoding: 'utf-8',
                    });
                    console.log();
