...
export class SessionCommand {
    static async run(themeManager, configManager, subaction, targetSessionId, sessionManager) {
        ...
        switch (action) {
            case 'list': {
                ...
                if (sessions.length === 0) {
                    ...
                    for (const s of sessions.slice(0, 10)) {
                        ...
                        console.log(`  • ${fmt.bold(fmt.primary(s.id))}  ${fmt.dimmed(dateStr)}  [${fmt.assistant(s.model)}]`);
                        if (s.workspaceRoot) {
            ...
            case 'resume': {
                ...
                if (!session) {
                    console.log();
                    console.log(fmt.error(targetSessionId
