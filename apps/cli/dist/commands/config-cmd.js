...
export class ConfigCommand {
    static async run(themeManager, configManager, subaction, key, value) {
        ...
        switch (action) {
            ...
            default: {
                ...
                console.log(`  • Runtime Engine:   ${fmt.primary(config.runtime?.default || 'mlx')}`);
                console.log(`  • Runtime Mode:     ${fmt.bold(config.runtime?.mode || 'local')}`);
                console.log();
                console.log(fmt.accent('SECURITY & PRIVACY'));
                console.log(`  • Privacy Tier:     ${fmt.bold(config.privacy?.mode || 'local')}`);
                console.log(`  • Cost Hard Limit:  $${config.cost_control?.hard_limit_usd ?? 10.0}`);
                console.log(`  • Confirmations:    ${fmt.dimmed(config.permissions?.risk_threshold || 'medium')}`);
                console.log();
