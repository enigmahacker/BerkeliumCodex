...
export class BkMatrix {
    static GLYPHS = '0123456789ABCDEFλπΩΨΣθβBk97✦◈◇⚡アイウエオカキクケコサシスセソタチツテト';
    /**
     ...
     */
    static renderCodexEmblem(themeManager) {
        ...
        const deepBlue = (t) => fgHex('#1e40af', t);
        const white = (t) => fgHex('#ffffff', t);
        const gold = (t) => fgHex('#f59e0b', t);
        const dim = (t) => fgHex('#64748b', t);
        const bold = (s) => (fmt ? fmt.bold(s) : `\x1b[1m${s}\x1b[22m`);
        console.log();
        console.log(cyan('             ') +
            gold(bold('97')) +
            cyan('               ╭───●───╮              ') +
     ...
     */
    static renderCodexHeader(themeManager, model, provider, workspace) {
        const fmt = themeManager.getFormatted();
        const cyan = (t) => fgHex('#00d2ff', t);
        const royal = (t) => fgHex('#3a7bd5', t);
        console.log();
     ...
     */
    static async playMatrixRain(themeManager, options = {}) {
...
//# sourceMappingURL=bk-matrix.js.map