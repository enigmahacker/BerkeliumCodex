...
import { CheckpointManager, BackgroundTaskManager, isValidAgentMode } from '@berkelium/agent';
import { PromptEngine, pickFileWithFinder } from '@berkelium/config';
import { NetworkController } from '@berkelium/tools';
import { RepoMapper, WorkingDirectoryManager, ProjectScanner, SystemScanner } from '@berkelium/context';
...
export class SlashCommandHandler {
    ...
    authStore;
    registry;
    constructor(runtime, themeManager, configManager, router, orchestrator, contextEngine, authStore) {
        ...
        switch (command) {
            ...
            case 'prompt': {
                ...
                if (subAction === 'reset') {
                    ...
                    console.log(fmt.success(layerName
                ...
                console.log(fmt.error(`Unknown system prompt action "${subAction}". Available: /system, /system set, /system reset, /system export`));
                return true;
            }
            case 'auth': {
                const subAction = subArgs[0]?.toLowerCase();
                ...
                if (subAction === 'login' || subAction === 'set') {
                    ...
                    if (!keyToSave) {
                        ...
                        try {
                            keyToSave = await rl.question(fmt.accent(`Enter API key for ${providerName.toUpperCase()}: `));
                        }
                        finally {
                            rl.close();
                        }
                    ...
                    await this.authStore.setApiKey(providerName, trimmedKey);
                    console.log(fmt.success(`✓ Successfully stored API key for ${providerName.toUpperCase()} in macOS Keychain / Secure Vault.`));
                    return true;
                }
                if (subAction === 'logout' || subAction === 'remove' || subAction === 'clear') {
                    ...
                    console.log(fmt.success(`✓ Removed credentials for ${providerName.toUpperCase()} from secure storage.`));
                    return true;
                }
                ...
                console.log(fmt.error(`Unknown auth action "${subAction}". Available: /auth, /auth login, /auth logout, /auth providers`));
                return true;
            }
            case 'matrix':
                if (arg === 'theme') {
            ...
            case 'perms': {
                const subAction = subArgs[0]?.toLowerCase();
                const pe = typeof this.orchestrator?.getPermissionEngine === 'function'
                ...
                if (subAction === 'ask' || subAction === 'auto' || subAction === 'full') {
                    pe.setPermissionLevel(subAction);
                ...
                console.log(`  Granular Grants:  ${grants.length > 0 ? grants.map((g) => `${g.capability} (${g.scope})`).join(', ') : 'None'}`);
            ...
            case 'mode': {
                ...
                console.log(`  Runtime Mode:   ${fmt.bold(fmt.accent(this.configManager.getConfig().runtime?.mode || 'local'))}`);
                console.log();
            ...
            case 't': {
                ...
                for (const t of tools) {
                    console.log(`  ${fmt.bold(t.metadata.name.padEnd(22, ' '))} [${fmt.accent(t.metadata.category)}] ${fmt.dimmed(t.metadata.description)}`);
                }
                console.log();
                return true;
            }
            ...
            case 'commit': {
                await this.runtime.executeTask(arg ? `Create a clean git commit with message: ${arg}` : 'Inspect git diff and commit the changes with a concise commit message.');
                return true;
            }
            case 'test': {
            ...
            case 'review':
                ...
                return true;
            case 'fix':
                await this.runtime.executeTask(arg ? `Fix the following issue: ${arg}` : 'Run diagnostics and tests, identify any failures, and fix them.');
                return true;
            case 'refactor':
                await this.runtime.executeTask(arg ? `Refactor: ${arg}` : 'Inspect the codebase architecture and suggest clean refactoring improvements.');
                return true;
            case 'explain':
                await this.runtime.executeTask(arg ? `Explain: ${arg}` : 'Provide a high-level architectural walkthrough of this repository.');
                return true;
            default:
