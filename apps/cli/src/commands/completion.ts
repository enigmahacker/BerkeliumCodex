...
export class CompletionEngine {
  private static cachedModels: CompletionOption[] | null = null;
  private static lastModelFetch = 0;

  /**
   * Resolve dynamic argument completion options for a given command and argument index.
   */
  public static async getArgumentSuggestions(
    context: CommandExecutionContext,
    command: CommandDefinition,
    argIndex: number,
    query: string
  ): Promise<ArgumentMatchResult[]> {
  ...
  private static async getModelOptions(context: CommandExecutionContext): Promise<CompletionOption[]> {
    const now = Date.now();
