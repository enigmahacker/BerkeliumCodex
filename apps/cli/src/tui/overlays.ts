...
import { PermissionPolicy, BerkeliumConfig, PromptLayers, PromptEngine } from '@berkelium/config';
...
export class TUIOverlays {
  ...
  public static renderSystemPrompt(
    ...
    const layerItems: Array<{ name: string; content?: string; isCustom: boolean }> = [
  ...
  public static renderPermissions(themeManager: ThemeManager, policy: PermissionPolicy): void {
    const fmt = themeManager.getFormatted();
  ...
  public static renderContext(themeManager: ThemeManager, bd: ContextBreakdown): void {
    const fmt = themeManager.getFormatted();
  ...
  public static renderModels(
    ...
    discovered: Record<string, ModelInfo[]>
  ): void {
    const fmt = themeManager.getFormatted();
  ...
  public static renderThemes(themeManager: ThemeManager): void {
    const fmt = themeManager.getFormatted();
  ...
  public static renderConfig(themeManager: ThemeManager, config: BerkeliumConfig): void {
    const fmt = themeManager.getFormatted();
  ...
  public static renderSecurityAudit(themeManager: ThemeManager, workspaceRoot: string): void {
    const fmt = themeManager.getFormatted();
