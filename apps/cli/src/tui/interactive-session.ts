...
import { InputStateMachine, InputStateResult } from './input-state-machine.js';
import { CommandPaletteRenderer } from './command-palette.js';
import { CommandExecutionContext } from '../commands/types.js';
...
export class InteractiveSession {
  ...
  private paletteRenderer: CommandPaletteRenderer;
  ...
  private async handleTTYKeypress(char: string | undefined, key: any): Promise<void> {
  ...
  private renderCurrentState(res?: InputStateResult): void {
    if (this.isProcessing) return;

    let paletteLines: string[] = [];
    ...
    if (shouldRender) {
      if (mode === 'SlashCommand' && res?.commandMatches) {
        paletteLines = this.paletteRenderer.renderCommandSuggestions(
          res.commandMatches,
          res.selectedIndex,
          res.scrollOffset,
          res.buffer
  ...
  private renderTTYPrompt(paletteLines: string[] = []): void {
    if (this.isProcessing) return;
  ...
  private async processInputLine(input: string): Promise<void> {
    ...
    if (detectedProvider) {
      ...
      console.log(
        fmt.success(
          `✓ Detected and registered ${detectedProvider.toUpperCase()} API key in macOS Keychain / Vault!`
        )
      );
      console.log(
        fmt.dimmed(
          `  Provider "${detectedProvider}" is now authenticated. Ready to code!`
        )
      );
      console.log();
      this.promptUser();
